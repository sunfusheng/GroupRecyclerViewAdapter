 ...
 */
public class WeChatNewMessageGroupAdapter extends HeaderGroupRecyclerViewAdapter<GroupData.WeChatItemConfig> {

    public WeChatNewMessageGroupAdapter(Context context) {
        super(context);
    }

    public WeChatNewMessageGroupAdapter(Context context, List<List<GroupData.WeChatItemConfig>> items) {
        super(context, items);
    }

    public WeChatNewMessageGroupAdapter(Context context, GroupData.WeChatItemConfig[][] items) {
        super(context, items);
    }
    ...
    public int getChildLayoutId(int viewType) {
        if (viewType == GroupData.VIEW_TYPE_WECHAT_SWITCH) {
            ...
            switchButton.setOnCheckedChangeListener((buttonView, isChecked) -> {
                holder.itemView.post(() -> {
                    if (R.attr.key_new_message_notify == item.key) {
                        if (isChecked) {
                            insertGroups(groupPosition + 1, GroupData.newMessageNotifyItemsHolder);
                        } else {
                        ...
                        if (isChecked) {
                            if (!hasItem(groupPosition, R.attr.key_new_message_voice)) {
                                insertItem(groupPosition, childPosition + 1, GroupData.WeChatItemConfig.NEW_MESSAGE_VOICE);
                            }
                        } else {
                            if (hasItem(groupPosition, R.attr.key_new_message_voice)) {
    ...
    private boolean hasItem(int groupPosition, int key) {
        List<GroupData.WeChatItemConfig> items = getGroups().get(groupPosition);
        if (!GroupAdapterUtils.isEmpty(items)) {
            for (GroupData.WeChatItemConfig item : items) {
                if (item.key == key) {
                    return true;
                }
            }
